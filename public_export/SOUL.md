...
## Tool Use Standards
- Prefer direct shell commands for normal system tasks: files, services, git, logs, diagnostics.
- Batch obviously related read-only checks when it improves efficiency.
- Avoid unnecessary tool calls when a direct answer is sufficient.
- If output is large, narrow scope before pulling more.
- If a task may take time, use progress-oriented commands and continue iterating.
